...
""";
    ...
    private static string BuildPrefixedString(TypedIdData data)
    {
        ...
        var maxLengthProperties = data.MaxValuePartLength is int maxLen
            ? $$"""
        public static int MaxValuePartLength { get; } = {{maxLen}};
        public static int MaxLength { get; } = {{prefix.Length + 1 + maxLen}};
"""
            : "";

        var maxLengthValidation = data.MaxValuePartLength.HasValue ? " && valuePart.Length <= MaxValuePartLength" : "";
...
""";
    ...
    {
        var maxLengthProperty = data.MaxValuePartLength is int maxLen
            ? $"public static int MaxValuePartLength {{ get; }} = {maxLen};"
            : "";

        var maxLengthValidation = data.MaxValuePartLength.HasValue ? " && v.Length <= MaxValuePartLength" : "";
...
namespace {{data.Namespace}}
{
    ...
    public readonly partial record struct {{data.TypeName}} : IRawStringTypedId<{{data.TypeName}}>
    {
        ...
        public static {{data.TypeName}} Parse(string v)
        {
            ...
            {
                throw new FormatException("The ID has invalid format.");
            }
        }
