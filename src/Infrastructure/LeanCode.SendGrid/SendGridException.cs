...
public class SendGridException : Exception
{
    ...
    internal SendGridException(HttpStatusCode responseStatusCode, IEnumerable<SendGridError>? errors)
        : base($"SendGrid request failed with status code {responseStatusCode}.")
    {
        ...
        if (errors is object)
        {
            ErrorMessages = errors.Select(e => e.Message!).Where(m => m is object).ToImmutableArray();
        }
    }
}
